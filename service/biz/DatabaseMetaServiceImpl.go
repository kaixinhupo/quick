...
import (
	...
	"github.com/kaixinhupo/quick/infrastructure/errors"
		...
		for _, tbl := range tbls {
			if dict[tbl.Name] == 1 {
				m := &model.TableMeta{
					...
					ModelNameFirstLower: getModelNameFirstLower(tbl.Name),
					BasePath:            getRoutePath(tbl.Name),
...
func (impl *DatabaseMetaServiceImpl) getFields(table *schemas.Table) []*model.FieldMeta {
	...
	for i, col := range cols {
		field := &model.FieldMeta{
			...
			ColTypeLower:      strings.ToLower(col.SQLType.Name),
			ColTypeLen:        getColTypeLen(col),
...
func getRoutePath(name string) string {
	return "/" + strcase.ToSnake(name)
}

func getColTypeLen(col *schemas.Column) string {
	var s string
	switch col.SQLType.Name {
	case "CHAR", "VARCHAR", "BINARY", "VARBINARY":
		s = fmt.Sprintf("(%d)", col.SQLType.DefaultLength)
	case "REAL", "FLOAT", "DOUBLE", "DECIMAL", "NUMERIC":
		s = fmt.Sprintf("(%d,%d)", col.SQLType.DefaultLength, col.SQLType.DefaultLength2)
	default:
		s = ""
	}
	return s
...
func getModelNameFirstLower(tableName string) string {
