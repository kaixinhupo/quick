...
func (impl *GencodeServiceImpl) Generate(meta []*model.TableMeta) error {
	...
	for _, m := range meta {
		ctx.Meta = m
		err := impl.generate(ctx, output)
		if err != nil {
			return err
		}
	}
	err = impl.zipDir(output, uid)
	if err != nil {
		return err
	}
...
func (c *GencodeServiceImpl) generate(ctx *GenContext, output string) error {
	err := c.generateInfo(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateEntity(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateModel(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateRepository(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateRepositoryImpl(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateService(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateServiceImpl(ctx, output)
	if err != nil {
		return err
	}
	err = c.generateController(ctx, output)
	if err != nil {
		return err
	}
	return nil
...
func (c *GencodeServiceImpl) generateByTemplate(path string, ctx *GenContext, template string) error {
	...
	err = c.engine.ExecuteWriter(buf, template, "", ctx)
	if err != nil {
		return err
	}
	return buf.Flush()
